...
/*
	...
	All parts inside the group have a pattern that define how they will contribute to rebuilding the value.
...
int parse_pattern(char *pattern, int **byte_offsets, int *n_src_bytes) {
	if(!pattern) {
		if(trace > 0) {
			printf("pattern not set, defaulting to \"0\" (8 bits)\n");
		}
		*n_src_bytes = 1;
...
int write_group(FILE *out, MD5_CTX *md5_ctx, t_part *part) {
	...
	for(i = 0; i < part->g.n_parts; i++) {
		...
		if(trace > 0) {
			printf("n_src_bytes[%d] = %d\n", i, n_src_bytes[i]);
		}
		if(trace > 0) {
			printf("bytes_offsets[%d][0] = %d\n", i, byte_offsets[i][0]);
		}
