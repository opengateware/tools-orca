...
int write_arc(t_mra *mra, char *filename) {
	...
	*/
	...
	if(i != -1 && mra->roms[i].n_parts == 1 && !mra->roms[i].parts[0].is_group && mra->roms[i].parts[0].p.data_length == 1) {
	...
	if(mra->rbf.name) {
		...
		if(strnlen(rbf, 32) > 8) {
			printf("warning: RBF file name may be too long for MiST\n");
		}
