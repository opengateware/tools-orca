...
int read_part(XMLNode *node, t_part *part, int parent_index) {
	...
	for(j = 0; j < node->n_attributes; j++) {
		...
		} else if(strncmp(node->attributes[j].name, "length", 7) == 0 || strncmp(node->attributes[j].name, "size", 5) == 0) {
...
int read_switches(XMLNode *node, t_switches *switches) {
	...
	for(i = 0; i < node->n_attributes; i++) {
		...
		if(strncmp(attr->name, "base", 10) == 0) {
			...
			if(n-- > 0) {
				switches->defaults |= (a & 0xff);
			}
			if(n-- > 0) {
				switches->defaults |= ((b & 0xff) << 8);
			}
			if(n-- > 0) {
				switches->defaults |= ((c & 0xff) << 16);
			}
			if(n-- > 0) {
